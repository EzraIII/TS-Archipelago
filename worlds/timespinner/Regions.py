...
def create_regions_and_locations(world: MultiWorld, player: int, precalculated_weights: PreCalculatedWeights):
    ...
    connect(world, player, 'Varndagroth tower left', 'Varndagroth tower right (upper)', logic.has_keycard_C or is_option_enabled(world, player, "GlitchesLogic"))
    connect(world, player, 'Varndagroth tower left', 'Varndagroth tower right (lower)', logic.has_keycard_B or is_option_enabled(world, player, "GlitchesLogic"))
    connect(world, player, 'Varndagroth tower left', 'Sealed Caves (Sirens)', lambda state: logic.has_keycard_B(state) and state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic"))
    ...
    connect(world, player, 'Varndagroth tower right (upper)', 'Varndagroth tower right (elevator)', lambda state: state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic") and state.has('Lightwall', player))
    ...
    connect(world, player, 'Varndagroth tower right (lower)', 'Varndagroth tower left', logic.has_keycard_B or is_option_enabled(world, player, "GlitchesLogic"))
    connect(world, player, 'Varndagroth tower right (lower)', 'Varndagroth tower right (elevator)', lambda state: state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic") and state.has('Lightwall', player))
    connect(world, player, 'Varndagroth tower right (lower)', 'Sealed Caves (Sirens)', lambda state: logic.has_keycard_B(state) and state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic"))
    connect(world, player, 'Varndagroth tower right (lower)', 'Military Fortress', lambda state: logic.can_kill_all_3_bosses or is_option_enabled(world, player, "GlitchesLogic") and state.has('Lightwall', player) and state.has('Celestial Sash', player) and state.has('Succubus Hairpin', player))
    connect(world, player, 'Varndagroth tower right (lower)', 'Space time continuum', logic.has_teleport)
    connect(world, player, 'Sealed Caves (Sirens)', 'Varndagroth tower left', lambda state: state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic") and logic.has_upwarddash(state))
    connect(world, player, 'Sealed Caves (Sirens)', 'Varndagroth tower right (lower)', lambda state: state.has('Elevator Keycard', player) or is_option_enabled(world, player, "GlitchesLogic") and logic.has_upwarddash(state))
    ...
    connect(world, player, 'Before the lab', 'The lab', logic.has_keycard_B or is_option_enabled(world, player, "GlitchesLogic") and (flooded.flood_lab or logic.has_doublejump))
    ...
    connect(world, player, 'The lab', 'The lab (upper)', lambda state: (state.can_reach('The lab (power off)', 'Region', player) or is_option_enabled(world, player, "GlitchesLogic")) and logic.has_forwarddash_doublejump(state)) # TODO: Extended Risky Dash Logic
    connect(world, player, 'The lab (upper)', 'The lab')
    connect(world, player, 'The lab (upper)', 'Emperors tower', logic.has_doublejump)
    ...
    connect(world, player, 'Skeleton Shaft', 'Sealed Caves (Xarion)', logic.has_keycard_A or is_option_enabled(world, player, "GlitchesLogic"))
    ...
    connect(world, player, 'Forest', 'Left Side forest Caves', lambda state: flooded.flood_lake_serene_bridge or state.has('Talaria Attachment', player) or logic.has_timestop(state)) # TODO: Extended Risky Dash Logic
    ...
    connect(world, player, 'Left Side forest Caves', 'Upper Lake Serene', logic.has_timestop or is_option_enabled(world, player, "GlitchesLogic"))
    connect(world, player, 'Left Side forest Caves', 'Lower Lake Serene', lambda state: not flooded.flood_lake_serene or state.has('Water Mask', player) or is_option_enabled(world, player, "GlitchesLogic"))
    ...
    connect(world, player, 'Lower Lake Serene', 'Caves of Banishment (upper)', lambda state: flooded.flood_lake_serene and state.has('Water Mask', player) or logic.has_doublejump(state))
    connect(world, player, 'Caves of Banishment (upper)', 'Lower Lake Serene', lambda state: not flooded.flood_lake_serene or state.has('Water Mask', player) or is_option_enabled(world, player, "GlitchesLogic") and (state.has('Talaria Attachment', player) or logic.has_doublejump(state)))
    ...
    connect(world, player, 'Caves of Banishment (Maw)', 'Caves of Banishment (upper)', lambda state: logic.has_doublejump(state) if not flooded.flood_maw else state.has('Water Mask', player)) # TODO: EnemyRando & TwoPlayerLogic Support
    connect(world, player, 'Caves of Banishment (Maw)', 'Caves of Banishment (Sirens)', lambda state: state.has_any({'Gas Mask', 'Talaria Attachment'}, player) and (not flooded.flood_maw or state.has('Water Mask', player)))
    connect(world, player, 'Caves of Banishment (Maw)', 'Space time continuum', lambda state: logic.has_teleport and (not flooded.flood_maw or state.has('Water Mask', player)))
    connect(world, player, 'Caves of Banishment (Sirens)', 'Forest')
    connect(world, player, 'Caves of Banishment (Sirens)', 'Caves of Banishment (Maw)', lambda state: is_option_enabled(world, player, "GlitchesLogic") and state.has('Lightwall', player) and (not flooded.flood_maw or state.has('Water Mask', player))) # TODO: EnemyRando Support
    ...
    connect(world, player, 'Royal towers (lower)', 'Royal towers', lambda state: state.has('Timespinner Wheel', player) or logic.has_forwarddash_doublejump(state) or is_option_enabled(world, player, "GlitchesLogic") and logic.has_doublejump(state))
    ...
    connect(world, player, 'Royal towers', 'Royal towers (upper)', logic.has_doublejump) # maybe TODO: TwoPlayerLogic & GlitchesLogic & Timestop
